...
impl Swapchain {
    ...
    pub fn create(
        ...
        let images = unsafe {
            swapchain
                ...
                .iter()
                .map(|image| {
                    ImageVariable::create_swapchain_image(Arc::clone(&context), *image, properties)
                })
                .collect::<Vec<_>>()
        };
    ...
    fn create_views(
        _device: &Device,
        swapchain_images: &[ImageVariable],
        _swapchain_properties: SwapchainProperties,
    ) -> Vec<vk::ImageView> {
        swapchain_images
            .iter()
            .map(|image| image.create_view(vk::ImageViewType::TYPE_2D, vk::ImageAspectFlags::COLOR))
            .collect::<Vec<_>>()
    }
